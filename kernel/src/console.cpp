...
void Console::print_line(char *str) {
  for (uint32_t i = 0; i < strlen(str); i++) {
    ...
    this->x_position += 8;
    if (this->x_position >= this->framebuffer->PixelsPerScanLine)
      this->new_line();
  }
}
...
void Console::put_char(char c, uint32_t x, uint32_t y, uint32_t color) {
  ...
  uint32_t *pixel = (uint32_t *)this->framebuffer->FrameBufferBase;
  ...
  for (uint32_t y = y_offset; y < y_offset + 16; y++) {
    for (uint32_t x = x_offset; x < x_offset + 8; x++) {
      if ((*glyph & (0b10000000 >> (x - x_offset))) > 0)
        *(pixel + x + (y * this->framebuffer->PixelsPerScanLine)) = color;
    }
    glyph++;
  }
}
...
void Console::clear_screen(uint32_t color) {
  uint32_t *pixel = (uint32_t *)this->framebuffer->FrameBufferBase;
  for (uint64_t i = 0; i < this->framebuffer->HorizontalRes * this->framebuffer->VerticalRes; i++)
    *(pixel + i) = color;
}
